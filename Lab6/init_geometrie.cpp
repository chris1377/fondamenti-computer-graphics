...
void crea_toro(Mesh* mesh, vec4 colore)
{
	...
	float R = 1.0, r = 0.3; //r raggio minore
	float s, t;
	...
	for (int i = 0; i <= Stacks; ++i) {
		...
		for (int j = 0; j <= Slices; ++j) {
			...
			// Calc The Vertex Positions
			float x = (R + r * cosf(phi)) * sinf(theta);
			float y = r * sinf(phi);
			float z = (R + r * cosf(phi)) * cosf(theta);

			// Push Back Vertex Data
			mesh->vertices.push_back(vec3(x, y, z));
			mesh->colors.push_back(colore);
			//Normale al vertice
			mesh->normals.push_back(vec3(normalize(vec3(-sin(theta) * sin(phi), cos(phi), -sin(phi) * cos(theta) ))));
			
			// Associa coordinate texture ai vertici
			s = U;
			t = V;  //TOBE DONE modify next line
			mesh->texCoords.push_back(vec2(s, t));//0.0, 0.0));
		}
	}
	...
	for (int nm = 0; nm < num_meshes; nm++)
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++) { 
			...
			if (mesh->mTextureCoords[0]) // does the mesh contain texture coordinates?
			{
				...
				mymesh[nm].texCoords.push_back(glm::vec2(vec.x, vec.y));
			}
			else
...
void add_obj(const char* name, int shaderType, vec3 translateVec, vec3 scaleVec, float angolo, vec3 rotation_axis, vector<MeshObj>& Model3D) {
	...
	for (int i = 0; i < nmeshes; i++)
	{
		...
		Model3D[i].material = MaterialType::BROWN;
